...
var amount = 20000; // Select amount of records to generate in parent cmdb.
...
function GenHost (){
    ...
    //  var prefix = capFirst(prefixes[getRandomInt(0, prefixes.length - 1)]);
    //  var suffix = capFirst(suffixes[getRandomInt(0, suffixes.length - 1)]);

	var RenGen = capFirst(prefixes[getRandomInt(0, prefixes.length +1)]) + '' + capFirst(suffixes[getRandomInt(0, suffixes.length + 1)]);
    var number = getRandomInt(1, 99999);

	// var name = capFirst(name1[getRandomInt(0, name1.length + 1)]) + ' ' + capFirst(name2[getRandomInt(0, name2.length + 1)]);

    return RenGen + "-" + number;
//return RenGen;
}
...
function executeRule(excludefield, genRecords, amount, prefix, type_of_array, PCF_mode, pop_index, c_fields, value, custom_IP, generatedIPs) {
    ...
    for (var i = 1; i <= amount; i++) {
        ...
        if (!arl.contains(excludefield, 'host_name')) {
            genRecords.setValue('host_name', GenHost ());
        }
